...
func newApp() *cli.App {
	...
	cliFlags := []cli.Flag{
		...
		&cli.StringFlag{
			Name:        "mps-shm-mount-path",
			Usage:       "Path where the MPS shm dir is mounted in the MPS control daemon pod.",
			Value:       "/driver-root/dev/shm",
			Destination: &flags.mpsShmMountPath,
			EnvVars:     []string{"MPS_SHM_MOUNT_PATH"},
		},
	}
